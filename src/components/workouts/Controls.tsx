...
export function Button({ label, onPress, secondary = false, busy = false, disabled = false, icon, destructive = false }: { label: string; onPress: () => void; secondary?: boolean; busy?: boolean; disabled?: boolean; icon?: keyof typeof Feather.glyphMap; destructive?: boolean }) {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={label} accessibilityState={{ disabled: busy || disabled, busy }} disabled={busy || disabled} onPress={onPress} style={({ pressed }) => [ws.button, secondary && ws.secondary, destructive && !secondary && { backgroundColor: colors.danger }, { opacity: busy || disabled ? 0.5 : pressed ? 0.8 : 1 }, pressed && !secondary && !destructive && { backgroundColor: colors.primaryDark }]}>
    {busy ? <ActivityIndicator color={foreground} /> : <>{icon && <ContextIcon name={icon} size={20} color={foreground} />}<Text numberOfLines={2} ellipsizeMode="tail" style={{ fontFamily: 'Inter_600SemiBold', fontSize: 15, lineHeight: 20, color: foreground, flexShrink: 1 }}>{label}</Text></>}
  </Pressable>;
}
...
export function Notice({ text, error = false, tone = 'info' }: { text: string; error?: boolean; tone?: 'info' | 'success' | 'warning' }) {
  ...
  return <View accessibilityRole="alert" style={[ws.sub, ws.row, { flexWrap: 'nowrap', alignItems: 'flex-start', borderWidth: 1, borderColor: `${color}40`, marginBottom: 12 }]}><ContextIcon name={icon} size={20} color={color} /><Text numberOfLines={8} ellipsizeMode="tail" style={[ws.text, { flex: 1 }]}>{text}</Text></View>;
}
export function Empty({ title, text, action, onAction }: { title: string; text: string; action: string; onAction: () => void }) {
  return <View style={[ws.card, { alignItems: 'center', paddingVertical: 32 }]}><ContextIcon name="book-open" size={40} color={colors.primary} /><Text style={[ws.title, { textAlign: 'center' }]}>{title}</Text><Text style={[ws.muted, { textAlign: 'center' }]}>{text}</Text><Button label={action} onPress={onAction} /></View>;
}
...
export function Sheet({ title, children, onClose, locked = false, footer }: { title: string; children: ReactNode; onClose: () => void; locked?: boolean; footer?: ReactNode }) {
  ...
  return <Modal visible transparent animationType="slide" statusBarTranslucent onRequestClose={() => !locked && onClose()}>
    <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end', paddingBottom: keyboardHeight }}>
      ...
      <View accessibilityViewIsModal style={{ maxHeight: maxSheetHeight, backgroundColor: colors.background, borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: 20, paddingBottom: keyboardHeight > 0 ? 16 : Math.max(insets.bottom, 20) }}>
        <View style={{ width: 36, height: 4, backgroundColor: colors.border, borderRadius: 4, alignSelf: 'center', marginBottom: 12 }} />
        <View style={[ws.row, { flexWrap: 'nowrap', marginBottom: 16 }]}><Text numberOfLines={2} ellipsizeMode="tail" style={[ws.title, { flex: 1, textTransform: 'uppercase' }]}>{title}</Text><Pressable accessibilityRole="button" accessibilityLabel="Đóng" disabled={locked} onPress={() => { Keyboard.dismiss(); onClose(); }} style={{ width: 44, height: 44, alignItems: 'center', justifyContent: 'center' }}><ContextIcon name="x" size={24} color={colors.text} /></Pressable></View>
        <ScrollView keyboardShouldPersistTaps="handled" keyboardDismissMode="interactive" showsVerticalScrollIndicator={true} style={{ flexShrink: 1 }} contentContainerStyle={{ gap: 16, paddingBottom: 16 }}>{children}</ScrollView>
