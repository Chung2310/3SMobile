    ...
    <View style={[styles.container, { paddingTop: Math.max(insets.top, 16) }]}>
      ...
      <View style={styles.topBar}>
        ...
        <View style={styles.titleWrap}>
          <Text style={styles.pageTitle}>Học viên của tôi</Text>
          <Text style={styles.pageSubtitle}>{customers.length} học viên đang quản lý</Text>
        </View>
        <View style={{ width: 40 }} />
      </View>
        ...
        {filtered.length ? (
          filtered.map((item) => (
            <Card key={item.customerId}>
              ...
              />
              <Row
                label="Số phiếu InBody"
                value={`${item.measurementCount || 0} phiếu`}
                icon="↻"
              />
            </Card>
          ))
