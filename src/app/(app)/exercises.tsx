...
export default function ExercisesScreen() {
  ...
  return (
    <LibraryIconContext.Provider value={true}>
      <Screen
        title="Thư viện bài tập"
        subtitle="Tra cứu kỹ thuật và xây dựng giáo án."
        onBack={() => router.navigate('/(app)/(tabs)')}
      >
        {allowed ? (
          <ExerciseLibrary key={session.user.id} />
        ) : (
          <Notice error text="Thư viện bài tập dành cho PT và quản trị viên." />
        )}
      </Screen>
    </LibraryIconContext.Provider>
  );
}
